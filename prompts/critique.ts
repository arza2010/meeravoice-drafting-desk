...
<task>Review the draft against the deterministic check results, the 10-point voice checklist, and one additional item (11) below. Return a verdict per item, quoting the offending line where relevant, and a revised draft that fixes every failure with the smallest possible change.</task>
...
- items must cover all 11 items above in order (n = 1 through 11), each with pass (boolean), quote (the specific offending text, or null if it passes), and fix (what you changed or would change, or null if it passes).
